...
# Models                                    
######################################################################### 
def sis_replicator(graph, max_time, params, local=False, beta_fun = 'exponential'):
        ...
        for idx, node_i in enumerate(G.nodes()):
            ...
            if local is True:       # If only considering local neighbors
                if n_infected_neighbors > 0:    # If there is infection in my neighbors
                    ...
                    p_inf_node_i = beta_cooperation(p_inf, np.sum(cur_game_nn==0)/N, )
...
def run_model(model, graph, params, n_iters=10, max_time=300, num_checkpoints=1,  local=False, beta_fun='exponential', path_to_save_checkpoints=None):

    df_list = []
    chk_point = 0
    for n_iter in range(n_iters):

        game_states, infection_states, payoffs = model(graph, max_time, params, local=local, beta_fun=beta_fun)
